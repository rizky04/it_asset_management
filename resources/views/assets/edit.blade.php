<x-tailadmin-app-layout :page="'assets'">
    <div class="mb-6">
        ...
        <h2 class="text-2xl font-semibold text-gray-800 dark:text-white/90">Edit Aset</h2>
        <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">{{ $asset->name }}</p>
    </div>
        ...
        <form method="POST" action="{{ route('assets.update', $asset) }}">
            ...
            <div class="grid grid-cols-1 gap-5 sm:grid-cols-2">
                ...
                <div>
                    ...
                    <input type="text" name="material" value="{{ old('material', $asset->material) }}"
                           class="h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 text-sm text-gray-800 focus:border-brand-300 focus:outline-hidden dark:border-gray-700 dark:text-white/90" />
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="pic" value="{{ old('pic', $asset->pic) }}"
                           class="h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 text-sm text-gray-800 focus:border-brand-300 focus:outline-hidden dark:border-gray-700 dark:text-white/90" />
                </div>
                ...
                <div>
                    ...
                    <input type="number" name="qty" value="{{ old('qty', $asset->qty) }}" min="0"
                ...
                <div>
                    ...
                    <input type="number" name="good" value="{{ old('good', $asset->good) }}" min="0"
                ...
                <div>
                    ...
                    <input type="number" name="broken" value="{{ old('broken', $asset->broken) }}" min="0"
                ...
                <div class="flex items-center gap-6 sm:col-span-2">
                    <label class="flex items-center gap-2 cursor-pointer">
                        <input type="checkbox" name="for_sale" value="1" {{ old('for_sale', $asset->for_sale) ? 'checked' : '' }}
                               class="h-4 w-4 rounded border-gray-300 text-brand-500 focus:ring-brand-500" />
                        <span class="text-sm text-gray-700 dark:text-gray-400">Dijual</span>
                    </label>
                    <label class="flex items-center gap-2 cursor-pointer">
                        <input type="checkbox" name="obsolete" value="1" {{ old('obsolete', $asset->obsolete) ? 'checked' : '' }}
                               class="h-4 w-4 rounded border-gray-300 text-brand-500 focus:ring-brand-500" />
